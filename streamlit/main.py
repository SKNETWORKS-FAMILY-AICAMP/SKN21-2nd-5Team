...
# Streamlit 페이지 설정은 스크립트 최상단에서 한 번만 호출되어야 합니다.
st.set_page_config(
    page_title="메인 앱",
    initial_sidebar_state="collapsed",
    layout="wide"  # layout="wide"를 여기에 추가
)
...
if not st.session_state.logged_in:
    ...
    if st.session_state.role == "admin":
        # -------------------- 1. 데이터 로드 및 전처리 --------------------
        @st.cache_data
        def load_data(file_name_parts=('..', 'data',"hotel_bookings.csv")): # 인자 이름을 좀 더 명확하게 변경
            try:
                # 튜플로 받은 경로 구성 요소를 os.path.join으로 연결하여 실제 파일 경로 문자열 생성
                file_path = os.path.join(*file_name_parts) # *를 사용하여 튜플 언팩
                df = pd.read_csv(file_path) # 수정된 file_path 변수 사용
                ...
                # 실제 취소되지 않은 예약에 대해서만 수익을 계산하는 것이 더 정확할 수 있습니다.
                ...
                st.error(f"🛑 오류: '{os.path.join(*file_name_parts)}' 파일을 찾을 수 없습니다. 파일을 확인해 주세요.")
                return pd.DataFrame()
        ...
        def generate_charts(filtered_data, dashboard_title):
            ...
            with col_a1:
                ...
                if filtered_data['hotel'].nunique() > 1:
                    ...
                    # --- 수정된 부분: groupby 결과에 바로 컬럼 할당하지 않고 DataFrame으로 변환 후 할당 ---
                    ...
                    overall_cancel_trend_df.columns = ['Month', 'Cancellation Rate', 'Hotel Type'] # 컬럼 이름 재설정
            ...
            with col_b1: # 이 부분을 col_b1으로 수정했습니다.
                ...
                fig_b1 = px.line(adr_trend, x='Month', y='Average ADR',
                                 title='월별 평균 일일 요금 (ADR)', markers=True,
                                 color_discrete_sequence=px.colors.qualitative.Pastel,
                                 height=400)
                fig_b1.update_yaxes(tickformat="$,.0f")
                st.plotly_chart(fig_b1, use_container_width=True)

            with col_b2: # 이 부분을 col_b2로 수정했습니다.
                ...
                fig_b2 = px.bar(revenue_by_segment, x='Market Segment', y='Total Estimated Revenue',
                                title='시장 부문별 총 예상 수익',
        ...
        def main_admin_dashboard():
            ...
            data = load_data() # <-- file_name_parts 인자를 기본값으로 사용
            if data.empty:
                return
